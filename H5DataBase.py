...
class H5DataBase:
    def __init__(self, h5FilePath: Path, tab_path=""):
    ...
    def __get_h5handle(h5FilePath, mode="r"):
        assert h5FilePath.suffix == ".h5", "h5FilePath must be a h5 file"
        assert h5FilePath.exists(), "{} not exists".format(h5FilePath)
    ...
    def __write_array_to_H5DataBase(
    ...
    def write_pivotDF_to_h5data(
        cls, h5FilePath: Path, pivotDF: pd.DataFrame, pivotKey: str, rewrite=False
    ):
        assert h5FilePath.suffix == ".h5", "h5FilePath must be a h5 file"
        ...
        with h5py.File(h5FilePath, mode) as file:
            ...
            cls.__write_array_to_H5DataBase(
                H5DataBase, meatDataGroup, "index", index_array
            )
            cls.__write_array_to_H5DataBase(
                H5DataBase, meatDataGroup, "columns", columns_array
            )
            # 写入pivot数据
            pivotDataGroup = file.create_group("pivotData")
            cls.__write_array_to_H5DataBase(
                H5DataBase, pivotDataGroup, pivotKey, data_array
            )
            # 关闭HDF5文件
            file.close()
    ...
    def add_pivotDF_to_h5data(
        cls, h5FilePath: Path, pivotDF: pd.DataFrame, pivotKey: str, reindex=False
    ):
        assert h5FilePath.suffix == ".h5", "h5FilePath must be a h5 file"
        assert h5FilePath.exists(), "{} not exists".format(h5FilePath)
        ...
        with h5py.File(h5FilePath, "a") as file:
            cls.__write_array_to_H5DataBase(
                H5DataBase, file, "pivotData/" + pivotKey, data_array
            )
            # 关闭HDF5文件
            file.close()

    def load_pivotDF_from_h5data(self, pivotKey: str, defualt=None):
        if pivotKey not in self.known_data:
            if defualt is not None:
                return defualt
            else:
                raise AttributeError(
                    "Attribute '{}' for {} not defined.".format(
                        pivotKey, self.h5FilePath
                    )
                )
